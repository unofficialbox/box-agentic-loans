...
class LoanSkillPackageTests(unittest.TestCase):
    def fixture(self, root):
        for name in ['skills/loan-origination-claude/SKILL.md', 'skills/loan-origination-quick/SKILL.md', 'skills/loan-origination-slack/SKILL.md', 'skills/loan-origination-gemini/SKILL.md']:
            target = root / name
    ...
    def test_gemini_archive_contains_only_its_entrypoint(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            self.fixture(root)
            output = package(root / 'gemini.skill', root, skill_name='loan-origination-gemini')
            with zipfile.ZipFile(output) as archive:
                self.assertEqual(set(archive.namelist()), {'loan-origination-gemini/SKILL.md'})
