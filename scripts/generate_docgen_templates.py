...
def add_row_table(doc, headers, cells, widths_dxa):
    """A header row plus one body row whose cells carry the tablerow tags."""
    ...
    for idx, header in enumerate(headers):
        ...
        shade(cell, LIGHT)
        set_font(cell.paragraphs[0].add_run(header), 10, True, DARK)
    for idx, value in enumerate(cells):
        cell = table.rows[1].cells[idx]
        cell.vertical_alignment = WD_CELL_VERTICAL_ALIGNMENT.CENTER
        shade(cell, "FFFFFF")
        set_font(cell.paragraphs[0].add_run(value), 10.5)
...
def commitment_letter_salesforce():
    ...
    tab exports (`sample-data/docgen/LOS_Loan__c_DocGen.json`). Tags therefore name
    Salesforce API fields (`{{LOS_Loan__c.Loan_ID__c}}`), walk lookups through `__r`
    relationships, iterate the borrower account's `Borrower_Loans__r` child list with a
    `tablerow`, and read the generating user from `$User`. It carries the same Box Sign
    fields as the first template so `prepareSignatureRequest` works on its output.
    """
    doc = Document()
    configure(doc, "LOS Commitment Letter (Salesforce)", "Acme Bank | {{" + SF + ".Loan_ID__c}}")
    add_title(
        doc,
        "Commitment letter",
        "Commitment Letter",
        "Generated by Box Doc Gen for Salesforce from loan record {{" + SF + ".Name}}",
    )
    add_key_values(doc, [
        ("Loan ID", "{{" + SF + ".Loan_ID__c}}"),
    ...
    add_key_values(doc, [
        ...
        ("Interest rate", "{{" + SF + ".Interest_Rate__c}}% per annum"),
        ("Term", "{{" + SF + ".Term_Months__c}} months"),
        ("Loan-to-value", "{{" + SF + ".LTV__c}}%"),
        ("Debt service coverage", "{{" + SF + ".DSCR__c}}x"),
        ("Collateral", "{{" + SF + ".Collateral_Type__c}}"),
        ("Collateral value", "USD {{" + SF + ".Collateral_Value__c :: format(\"US-Number\")}}"),
        ("Target closing date", "{{" + SF + ".Target_Closing_Date__c}}"),
        ("Maturity date", "{{" + SF + ".Maturity_Date__c}}"),
        ("Risk rating", "{{" + SF + ".Risk_Rating__c :: optional}}"),
    ])
    doc.add_heading("Underwriting notes", level=2)
    set_font(doc.add_paragraph().add_run("{{ if " + SF + ".Underwriting_Notes__c isPresent }}"), 11)
    set_font(doc.add_paragraph().add_run("{{" + SF + ".Underwriting_Notes__c}}"), 11)
    ...
    add_key_values(doc, [
        ("Applicant", "{{" + SF + ".Applicant_Name__c}} ({{" + SF + ".Applicant_Email__c}})"),
        ("Industry", "{{" + SF + ".Borrower_Account__r.Industry :: optional}}"),
        ("Billing address", "{{" + SF + ".Borrower_Account__r.BillingStreet}}, {{" + SF + ".Borrower_Account__r.BillingCity}}, "
                            "{{" + SF + ".Borrower_Account__r.BillingState}} {{" + SF + ".Borrower_Account__r.BillingPostalCode}}"),
        ("Opportunity", "{{" + SF + ".Opportunity__r.Name :: optional}} ({{" + SF + ".Opportunity__r.StageName :: optional}})"),
    ])
    doc.add_heading("Loans on record for this borrower", level=2)
    add_row_table(
        doc,
        ["Loan ID", "Type", "Amount (USD)", "Status"],
        [
            "{{tablerow loan in " + SF + ".Borrower_Account__r.Borrower_Loans__r}}{{loan.Loan_ID__c}}",
            "{{loan.Loan_Type__c}}",
            "{{loan.Loan_Amount__c :: format(\"US-Number\")}}",
            "{{loan.Status__c}}{{endtablerow}}",
        ],
        [2340, 2340, 2340, 2340],
    )
    doc.add_heading("Prepared by", level=2)
    add_key_values(doc, [
        ("Loan officer", "{{" + SF + ".Loan_Officer__r.Name}}, {{" + SF + ".Loan_Officer__r.Title :: optional}}"),
        ("Loan officer contact", "{{" + SF + ".Loan_Officer__r.Email}} | {{" + SF + ".Loan_Officer__r.Phone :: optional}}"),
