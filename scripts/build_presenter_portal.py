...
PAGES = (
    PresenterPage("00-operator-setup-guide.html", "Operator Setup", "Prepare", "Environment readiness", "Configure and validate the confirmed environment."),
    PresenterPage("01-box-salesforce-los-guide.html", "Loan Origination Clickpath", "Present", "Canonical prompts", "Read evidence, confirm record updates, generate a draft, and demonstrate signature refusal."),
    PresenterPage("02-rehearsal-guide.html", "Stage, Rehearsal and Reset", "Rehearse", "Operational discipline", "One harness, two windows, timed passes, and owned cleanup."),
)
...
def page_class(page: PresenterPage) -> str:
    return {
        "Prepare": "prepare",
        "Present": "scenario-one",
        "Rehearse": "communicate",
    }[page.group]
...
def build_landing(output: Path | None = None) -> Path:
    ...
    group_copy = {
        ...
        "Present": "The same canonical prompts as the Markdown clickpath.",
        "Rehearse": "Timed passes, explicit evidence and owned cleanup.",
    }
...
<title>Acme LOS · Presenter Library</title><style>{SHARED_CSS}</style></head>
<body><header class="masthead"><div class="shell masthead-inner">
  <div class="brandline"><div class="brand"><span class="brand-mark">LOS</span><span>Acme Loan Operations</span></div><span class="status">Portable presenter library · {len(PAGES)} chapters</span></div>
  <p class="kicker">Start here</p><h1>Choose the right story for the room.</h1>
  <p class="intro">One entry point for environment setup, the canonical demo clickpath, and rehearsal operations. Every chapter remains available as its own portable file.</p>
...
<main id="library" class="shell content">
  <aside class="combined-callout"><div><h2>Need one file?</h2><p>The combined edition packages all {len(PAGES)} chapters into a single self-contained HTML file with chapter navigation, keyboard controls, and no network dependencies.</p></div><a class="button" href="06-complete-presenter-edition.html">Launch combined edition {ARROW}</a></aside>
  {"".join(sections)}
</main><footer><div class="shell"><span>Acme LOS · Portable specification and local deterministic fixture</span><span>Markdown remains authoritative · HTML is the sharing layer</span></div></footer></body></html>'''
...
def build_combined(source_dir: Path | None = None, output: Path | None = None) -> Path:
    ...
    document = f'''<!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Acme LOS · Complete Presenter Edition</title><style>
{SHARED_CSS}
...
<header class="reader-head"><div class="reader-brand"><span class="brand-mark">LOS</span><span>Complete Presenter Edition</span></div>
