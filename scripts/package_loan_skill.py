#!/usr/bin/env python3
"""Package the self-contained loan skill as a single-file Claude archive."""
import argparse
...
def package(output: Path, root: Path = ROOT) -> Path:
    skill = root / 'skills/loan-origination'
    ...
    with zipfile.ZipFile(output, 'w', zipfile.ZIP_DEFLATED) as archive:
        for source in files:
            archive.write(source, Path('loan-origination') / source.relative_to(skill))
    return output
