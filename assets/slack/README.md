...
Colours are the portal tokens only: `#0A332E` (ground), `#0F4C45` (mark), `#C9871A` (amber). The ground fades to a flat `#0A332E` at the edges, the same value as the manifest `background_color`, so the icon and the Slack app tile read as one surface. The tile has 260px (13%) rounded corners and a soft drop shadow, and is inset about 3.5% over an opaque `#0A332E` canvas: Slack flattens transparency onto white, so the canvas must be filled with the tile colour rather than left clear. The mark clears the corner radius with room to spare.
