...
The loan agent backend also needs the app's consumer secret, and signs in to Box itself: in the Box Admin Console → Integrations → **Box MCP Server**, add `http://localhost:8787/oauth/box/callback` as a redirect URI and copy the client ID and secret from Integration Credentials. They go in `.env` as `LOS_MCP_CLIENT_ID`/`LOS_MCP_CLIENT_SECRET` and `BOX_MCP_CLIENT_ID`/`BOX_MCP_CLIENT_SECRET`, with the org's My Domain URL (Setup → My Domain) as `LOS_MCP_LOGIN_URL`.
