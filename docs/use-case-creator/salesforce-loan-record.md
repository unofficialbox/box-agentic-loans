...
The main intake is the borrower's own: a Customer Community User signs in to the Acme Borrower Portal, fills in the application form, and uploads the documents the checklist for that loan type asks for (`config/los/required-documents.bcl`). Two governed actions serve it.
